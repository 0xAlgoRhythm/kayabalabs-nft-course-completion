 ...
 * - $0.50 minting fee
 * - Stores student name and details
...
contract KayabaCourseCompletionNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    /**
     * @dev Mint course completion certificate with full student details
     * @param to Student's wallet address
     * @param name Student's full name
     * @param email Student's email (can be empty string if you don't want to store)
     ...
     * @param grade Final grade (e.g., "A", "95%", "Distinction")
     * @param metadataURI Specific metadata URI (e.g., "0.json" or full IPFS path)
     */
    ...
    /**
     * @dev Get student name for a token (quick lookup)
     */
    function getStudentName(uint256 tokenId) public view returns (string memory) {
        require(_ownerOf(tokenId) != address(0), "Token does not exist");
        return studentInfo[tokenId].name;
    }

    /**
     * @dev Get all certificates issued to a specific wallet address
     * Note: This requires external indexing or looping through all tokens
     * For production, use The Graph or similar indexing service
     */
